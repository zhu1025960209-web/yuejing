...
class CycleWidgetProvider : AppWidgetProvider() {
    companion object {
        private const val ACTION_UPDATE = "com.example.yuejing.UPDATE_WIDGET"
        private const val ACTION_REFRESH = "com.example.yuejing.REFRESH_WIDGET"
    ...
    private fun setupWidgetBackground(
        ...
        try {
            // 获取小部件的默认尺寸
            val width = context.resources.getDimensionPixelSize(android.R.dimen.app_icon_size) * 4 // 4个图标宽度
            val height = context.resources.getDimensionPixelSize(android.R.dimen.app_icon_size) * 2 // 2个图标高度
    ...
    private fun setupClickEvents(context: Context, views: RemoteViews) {
        ...
        // 刷新按钮点击事件 - 手动刷新
        val refreshIntent = Intent(context, CycleWidgetProvider::class.java)
    ...
    private fun loadWidgetSettings(context: Context): Triple<String, Int, String?> {
        try {
            val file = java.io.File(context.filesDir, "widget_settings.json")
            if (!file.exists()) {
                return Triple("粉色 (默认)", 0xFFF5D8E4.toInt(), null)
            }
            val settingsJson = file.readText()
            val settings = kotlinx.serialization.json.Json.decodeFromString<WidgetSettings>(settingsJson)
